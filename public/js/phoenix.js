...
(function(){
  ...
  function getEdges(pts) {
    ...
    for (var i = 1; i <= 6; i++) edges.push([i, i === 6 ? 1 : i + 1]);
    for (var i = 1; i <= 6; i++) {
      var oi = i + 6;
      edges.push([i, oi]);
      edges.push([i, i === 6 ? 7 : oi + 1]);
    }
    for (var i = 7; i <= 12; i++) edges.push([i, i === 12 ? 7 : i + 1]);
    for (var i = 7; i <= 12; i++) {
      var skip = i + 2;
      if (skip > 12) skip -= 6;
      edges.push([i, skip]);
  ...
  function createParticle(edges, pts) {
    ...
    return { edge: e, t: Math.random(), speed: Math.random() * 0.004 + 0.002, color: colors[Math.floor(Math.random() * colors.length)], tail: [] };
  }
  ...
  function drawGeometry() {
    ...
    for (var i = 0; i < 13; i++) {
      ...
      ctx.fillStyle = grad;
      ctx.beginPath(); ctx.arc(pts[i][0], pts[i][1], r*1.8, 0, Math.PI*2); ctx.fill();
      ctx.strokeStyle = 'rgba(200,180,150,' + (baseAlpha + 0.08) + ')';
      ctx.lineWidth = 2.5;
      ctx.beginPath(); ctx.arc(pts[i][0], pts[i][1], r, 0, Math.PI*2); ctx.stroke();
      ...
      ctx.beginPath(); ctx.arc(pts[i][0], pts[i][1], r*0.25, 0, Math.PI*2); ctx.fill();
    }
  ...
  function initPhoenix() {
    var newCanvas = document.getElementById('phoenix-canvas');
    if (!newCanvas) return;
    ...
    time = 0;
    resize();
    var p = getPoints(); var e = getEdges(p);
    lightParticles = [];
    for (var i = 0; i < NUM_PARTICLES; i++) lightParticles.push(createParticle(e, p));
    animate();
  }

  window.addEventListener('resize', function(){ resize(); });
  window.initPhoenix = initPhoenix;

  // Auto-init on first load if canvas is present.
